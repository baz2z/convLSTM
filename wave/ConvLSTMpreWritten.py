...
class LSTM_cell(nn.Module):
    ...
    def __init__(self, x_channels, h_channels, lateral_channels):
        ...
        # wenn h auch curch 1x1: hier noch h in input einfügen
...
class Sequence(nn.Module):
    ...
    def __init__(self, in_channels, h_channels, lateral_channels):
        ...
        self.transition = nn.Conv2d(self.in_channels, self.lateral_channels, 3, padding="same")
        self.lstm1 = LSTM_cell(self.lateral_channels, self.h_channels, self.lateral_channels)
    ...
    def forward(self, x, steps=1):
        ...
        for t in range(steps):
            # x und h in transition weil falls davor wissen was x ist wird h nicht verwendet (vllt als vgl noch in arbeit rein)
            # v2: x und h in lstm concat
            l = self.transition(x)
            h, c = self.lstm1(l, h, c)
            #v2: h, c = self.lstm1(x, h, c)
