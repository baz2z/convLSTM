...
for index, row in df.iterrows():
    ...
    if modelName == "baseline":
        if mp == 1 and paramLevel == 1:
            ...
            ax1.errorbar(testSpeed, loss170, yerr=var, ecolor=col)
    elif modelName == "lateral":
        if mp == 1 and paramLevel == 2:
            ax1.scatter(testSpeed + scatter, loss170, color=col, s=16, alpha=0.7)
            ax1.errorbar(testSpeed + scatter, loss170, yerr=var, ecolor=col)
        ...
        if mp == 2 and paramLevel == 2:
            ax1.scatter(testSpeed + 2*scatter, loss170, color=col, s=16, alpha=0.7)
            ax1.errorbar(testSpeed + 2*scatter, loss170, yerr=var, ecolor=col)
        if mp == 0.5 and paramLevel == 2:
            ax1.scatter(testSpeed - 2*scatter, loss170, color=col, s=16, alpha=0.7, marker="s")
            ax1.errorbar(testSpeed - 2*scatter, loss170, yerr=var, ecolor=col)
...
chocolate_line = mlines.Line2D([], [], color='purple', marker='s',
