...
def calcHorizonLoss(model, context, horizon, dataloader, og = False):
...
if __name__ == '__main__':
    ...
    for d in [datasetLoader3]:
        d.mu = waveMu
        d.std = waveStd
    dataloader3 = DataLoader(dataset=datasetLoader3, batch_size=100, shuffle=False, drop_last=True,
                            collate_fn=lambda x: default_collate(x).to(device, torch.float))

    df = pd.DataFrame(columns=["name", "mult", "param", "paramExact", "horizonLoss"])# , "loss40_og", "loss70_og", "loss170_og"])
    ...
    for mult in [0.5, 1, 2]:
        for modelName in ["baseline", "lateral", "twoLayer", "skip", "depthWise"]:
            ...
            for param in [1, 2, 3]:
                ...
                if modelName == "baseline":
                    df.loc[counter] = [modelName, multBase, param, paramExact, horizonLoss]# , loss40_og, loss70_og, loss170_og]
                elif modelName == "depthWise":
                    df.loc[counter] = [modelName, multDw, param, paramExact, horizonLoss]# , loss40_og, loss70_og, loss170_og]
                else:
                    df.loc[counter] = [modelName, mult, param, paramExact, horizonLoss]# , loss40_og, loss70_og, loss170_og]
                counter += 1
                pathBack = f'../../../../../../plots'
                os.chdir(pathBack)
